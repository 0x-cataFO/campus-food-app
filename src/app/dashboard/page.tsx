...
import { Package, ShoppingBag, DollarSign, LogOut, Edit, Trash, Menu, Store, Pause, Play } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  // Safety check to break redirect loops: 
  // If no session, we show a message instead of a forced redirect.
  if (!session?.user?.id) {
    return (
  ...
  // --- ONBOARDING SCREEN FOR NEW VENDORS ---
  if (!vendor) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl max-w-md w-full text-center border border-slate-100">
          ...
          <p className="text-slate-500 mb-8">
            Set up your store and start taking orders from students today.
          </p>
          <AddFoodModal />
        </div>
      </div>
  ...
  const pendingOrdersCount = vendor.orders?.filter((order) => order.status === "PENDING").length || 0;
  const totalRevenue = vendor.orders?.reduce((sum, order) => sum + order.totalAmount, 0) || 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
          ...
          <SheetContent side="left" className="w-64 bg-white p-6 flex flex-col h-full">
            ...
            <nav className="space-y-2 flex-1">
              <Button variant="secondary" asChild className="w-full justify-start gap-2">
                <Link href="/dashboard"><Package className="w-4 h-4" /> Overview</Link>
              </Button>
              <Button variant="ghost" asChild className="w-full justify-start gap-2 text-slate-500">
                <Link href="#menu-items"><ShoppingBag className="w-4 h-4" /> Menu Items</Link>
              </Button>
              <Button variant="ghost" asChild className="w-full justify-start gap-2 text-slate-500">
                <Link href="#active-orders"><DollarSign className="w-4 h-4" /> Active Orders</Link>
              </Button>
            </nav>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto scroll-smooth">
        ...
        <div id="active-orders" className="mb-12 space-y-4 pt-4">
          <h2 className="text-xl font-bold tracking-tight text-slate-900">Active Orders</h2>
                ...
                <div key={order.id} className="bg-white border-2 border-[#FFD100]/20 shadow-sm rounded-2xl p-5 flex flex-col">
                  <div className="flex justify-between items-start mb-4 border-b pb-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Order ID: #{order.id.slice(-6).toUpperCase()}</p>
                    </div>
        ...
        {/* --- MENU ITEMS GRID --- */}
        <div id="menu-items" className="space-y-4 pt-4">
          <h2 className="text-xl font-bold tracking-tight">Your Menu</h2>
          {menuItems.length === 0 ? (
                  ...
                  <CardContent className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      ...
                      <div className="flex gap-2">
                        ...
                        {/* 1. NEW: Pause/Play Toggle Button */}
                        <form action={async () => {
                          "use server";
